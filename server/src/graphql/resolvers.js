...
const resolvers = {
    ...
    Mutation: {
        ...
        registerAffiliate: async (_, { name, email, refId, totalClicks, password, totalCommissions, }) => {
            try {
        ...
        updateAffiliate: async (_, { id, name, email, refId, totalClicks, totalCommissions, }) => {
            try {
                return await Affiliate.findOneAndUpdate({ _id: id }, {
        ...
        logClick: async (_, { refId }) => {
            try {
                const updatedAffiliate = await Affiliate.findOneAndUpdate({ refId }, { $inc: { totalClicks: 1 } }, { new: true });
                if (!updatedAffiliate) {
                    throw new Error("Affiliate not found");
                }
                return true;
